...
def options(request):
    ...
    return Response({'facilities': facilities, 'organisms': organisms, 'antibiotics': antibiotics})
...
def counts_summary(request):
    ...
    return Response({'total': total, 'patients': patients, 'facilities': facilities, 'pctR': pctR, 'R':R,'I':I,'S':S})
...
def sex_age(request):
    ...
    return Response({'by_sex':by_sex,'by_age':by_age})
...
def create_entry(request):
    ...
    if missing:
        return Response({"error": f"Missing required: {', '.join(missing)}"}, status=400)
